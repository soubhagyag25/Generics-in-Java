class node<T> {
    T data;
    node<T> next;
    node(T data) // Parameterized constructor to assign value
     {
         this.data = data;
         this.next = null;
    }
}
class list<T> { // Class 2 Helper class ( Generic LinkedList class)
    node<T> head;// Generic node instance
    private int length = 0;// Data member to store length of list
    list() { this.head = null; } // Default constructor
    void add(T data)// Method To add node at the end of List
    {
        node<T> temp = new node<>(data);// Creating new node with given value
        if (this.head == null) { // Checking if list is empty and assigning new value to head node
            head = temp;
        }
